    ...
    pub fn push_back(&mut self, elem: T) {
        ...
        match self.back {
            Some(last_node) => unsafe {
                (*last_node.as_ptr()).back = Some(new_node);
                (*new_node.as_ptr()).front = Some(last_node);
            },
            None => self.front = Some(new_node),
        }
...
impl<'a, T> CursorMut<'a, T> {
    ...
    pub fn move_next(&mut self) {
        if let Some(current) = self.cur {
            self.cur = unsafe { (*current.as_ptr()).back };
            if self.cur.is_some() {
    ...
    pub fn move_prev(&mut self) {
        if let Some(current) = self.cur {
            self.cur = unsafe { (*current.as_ptr()).front };
            if self.cur.is_some() {
                *self.index.as_mut().unwrap() -= 1;
            } else {
                self.index = None;
    ...
    pub fn peek_next(&mut self) -> Option<&mut T> {
        unsafe {
            let next = if let Some(current) = self.cur {
                (*current.as_ptr()).back
            } else {
                self.list.front
            };
    ...
    pub fn peek_prev(&mut self) -> Option<&mut T> {
        unsafe {
            let prev = if let Some(current) = self.cur {
    ...
    pub fn split_before(&mut self) -> LinkedList<T> {
        if let Some(current) = self.cur {
            ...
            let previous = unsafe { (*current.as_ptr()).front };
    ...
    pub fn split_after(&mut self) -> LinkedList<T> {
        if let Some(current) = self.cur {
            let old_len = self.list.len;
            let old_index = self.index.unwrap();
            let next_node = unsafe { (*current.as_ptr()).back };

            let new_len = old_index + 1;
            let new_front = self.list.front;
            let new_back = self.cur;
            let new_index = Some(new_len - 1);

            let output_len = old_len - new_len;
            let output_front = next_node;
            let output_back = self.list.back;

            if let Some(next_node) = next_node {
                unsafe {
                    (*next_node.as_ptr()).front.take();
    ...
    pub fn splice_before(&mut self, mut input: LinkedList<T>) {
        unsafe {
            if input.is_empty() {
            ...
            } else if let Some(current) = self.cur {
                let in_front = input.front.take().unwrap();
                let in_back = input.back.take().unwrap();

                if let Some(previous) = (*current.as_ptr()).front {
                    // general case, no boundries
                ...
                *self.index.as_mut().unwrap() += input.len;
                ...
                // curret is a ghost but non-empty, append at the back
                let in_front = input.front.take().unwrap();
                let in_back = input.back.take().unwrap();

                (*back.as_ptr()).back = Some(in_front);
                (*in_back.as_ptr()).front = Some(back);
...
fn assert_properties() {
    ...
    is_send::<std::io::Cursor<i32>>();
    is_sync::<std::io::Cursor<i32>>();
    ...
    /// ```compile_fail,E0308
    /// use lists::unsafe_dqueue::IterMut;
    ///
...
mod test {
    ...
    #[test]
    fn test_split() {
        let mut list: LinkedList<u32> = (0..10).collect();
